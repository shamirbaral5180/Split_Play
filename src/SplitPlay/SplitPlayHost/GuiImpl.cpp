...
HWND SplitPlayHost::splitPlayHostHwnd{0};
...
int SplitPlayHost::ShowGuiImpl()
{
    ...
    if (!RegisterClass(&wc))
        return 1;
    Hwnd = CreateWindowW(wc.lpszClassName, L"SplitPlay", WS_OVERLAPPEDWINDOW | WS_VISIBLE, 0, 0, 1100, 800, 0, 0, hInstance, 0);

    // Fit the window to the work area so nothing is off-screen on small displays
    if (Hwnd != nullptr)
    {
        RECT workArea{};
        if (SystemParametersInfoW(SPI_GETWORKAREA, 0, &workArea, 0))
        {
            ...
            int w = min(1180, workW - 20);
            int h = min(900, workH - 20);
            if (w < 640) w = workW;
            if (h < 480) h = workH;

            SetWindowPos(Hwnd, nullptr, workArea.left + (workW - w) / 2, workArea.top + (workH - h) / 2,
                         w, h, SWP_NOZORDER | SWP_NOACTIVATE);
        }
    }

    splitPlayHostHwnd = Hwnd;
    ...
    SplitPlayHost::ApplyModernStyle();

    wglMakeCurrent(g_HDCDeviceContext, g_GLRenderContext);

    //Fonts
    SplitPlayHost::LoadModernFonts(io);

...
SplitPlayInstanceHandle EasyHookInjectImpl(unsigned long pid, const wchar_t* dllFolderPath, bool stealth)
{
	const bool is64 = Isx64(pid);
	std::cout << "Target is " << (is64 ? "64-bit" : "32-bit") << std::endl;

	auto processPath = std::wstring(dllFolderPath) + (is64 ? L"SplitPlayIJ64.exe" : L"SplitPlayIJ32.exe");

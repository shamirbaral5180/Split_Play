	...
	if (rawInputState.sendMouseWheelMessages)
	{
		if((data.usButtonFlags & RI_MOUSE_WHEEL) != 0)
		{
			const unsigned int wparam = (data.usButtonData << 16)
				| MouseWheelFilter::splitPlaySignature
				| mouseMkFlags;
...
DWORD WINAPI RawInputWindowThread(LPVOID lpParameter)
{
	...
	const wchar_t* className = L"SPLITPLAYRAWINPUT";
	...
	if (RegisterClass(&wc))
	{
		RawInput::rawInputHwnd = CreateWindowW(
			wc.lpszClassName,
			L"SplitPlay Input: Raw Input window",
			0,
			0, 0, 300, 300,
			nullptr, nullptr,
			hinstance,
...
void RawInput::RegisterSplitPlayForRawInput()
{
	int tryCount = 0;

...
void HandleSelectableDualList(std::vector<T>& selected, std::vector<T>& deselected)
{
    ...
    std::vector<typename std::vector<T>::const_iterator> removeA{};
    std::vector<typename std::vector<T>::const_iterator> removeB{};
        ...
        if (ImGui::BeginTabBar("Tabs"))
        {
            ...
            if (ImGui::BeginTabItem("Message filter"))
            {
                if (ImGui::BeginTabBar("Filter tabs"))
                {
                    ...
                    {
                        if (!HookManager::IsInstalled(SplitPlayHookIDs::MessageFilterHookID))
                        {
                            ImGui::PushID(1337);
                            ImGui::PushStyleColor(ImGuiCol_Text,

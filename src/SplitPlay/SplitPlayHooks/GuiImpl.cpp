...
HWND SplitPlay::SplitPlayGuiHwnd = nullptr;
int     g_display_w = 800;
int     g_display_h = 600;
...
unsigned long SplitPlay::GuiThreadID = 0;
...
void SplitPlay::ToggleWindow()
{
    SetWindowVisible(!IsWindowVisible(SplitPlayGuiHwnd));
}

void SplitPlay::SetWindowVisible(bool visible)
{
    RawInput::rawInputState.guiOpened = visible;
    ShowWindow(SplitPlayGuiHwnd, visible ? SW_SHOW : SW_HIDE);
}

void SplitPlay::ToggleConsole()
{
    SetConsoleVisible(!IsWindowVisible((HWND)ConsoleHwnd));
}

void SplitPlay::SetConsoleVisible(bool visible)
{
    ShowWindow((HWND)ConsoleHwnd, visible ? SW_SHOW : SW_HIDE);
}

int SplitPlay::ShowGuiImpl()
{
    ...
    const wchar_t* className = L"SPLITPLAYUI";
    wc.lpszClassName = className;
    wc.style = CS_OWNDC | CS_NOCLOSE; 
    if (!RegisterClass(&wc))
        return 1;
    SplitPlayGuiHwnd = CreateWindowW(wc.lpszClassName, L"SplitPlay", WS_OVERLAPPEDWINDOW | WS_VISIBLE, 0, 0, 950, 700, 0, 0, hInstance, 0);

    // Show the window
    // ShowWindow(SplitPlayGuiHwnd, SW_SHOWDEFAULT);
    ShowWindow(SplitPlayGuiHwnd, SW_HIDE);
    UpdateWindow(SplitPlayGuiHwnd);
    ...
    //Init Win32
    ImGui_ImplWin32_Init(SplitPlayGuiHwnd);
    ...
    while (msg.message != WM_QUIT)
    {
    	...
    	if (PeekMessage(&msg, SplitPlayGuiHwnd, 0U, 0U, PM_REMOVE))
        //if (GetMessage(&msg, splitPlayHwnd, 0U, 0U))
        ...
        if (IsWindowVisible(SplitPlayGuiHwnd))
        {
...
void CreateGlContext()
{
    ...
    g_HDCDeviceContext = GetDC(SplitPlay::SplitPlayGuiHwnd);

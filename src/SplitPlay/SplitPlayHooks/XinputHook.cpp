...
inline DWORD WINAPI XInputGetState_Inline(DWORD dwUserIndex, XINPUT_STATE* pState, bool extended)
{
	...
	{
		return OpenXinput::SplitPlayOpenXinputGetState(targetControllerIndex, pState, extended);
	}
...
DWORD WINAPI Hook_XInputSetState(DWORD dwUserIndex, XINPUT_VIBRATION* pVibration)
{
	...
	if (XinputHook::useOpenXinput)
	{
		return OpenXinput::SplitPlayOpenXinputSetState(targetControllerIndex, pVibration);
	}
...
DWORD WINAPI Hook_XInputGetCapabilities(DWORD dwUserIndex, DWORD dwFlags, XINPUT_CAPABILITIES* pCapabilities)
{
	...
	if (XinputHook::useOpenXinput)
	{
		return OpenXinput::SplitPlayOpenXinputGetCapabilities(targetControllerIndex, dwFlags, pCapabilities);
	}
...
void SelectDinputDevice()
{
	...
	else
	{
		...
		{
			dinputDevice->SetCooperativeLevel(SplitPlay::SplitPlayGuiHwnd, DISCL_BACKGROUND | DISCL_NONEXCLUSIVE);

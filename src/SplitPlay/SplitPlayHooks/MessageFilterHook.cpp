...
{	
	...
	if (!MessageFilterAllow<SPLITPLAY_MESSAGE_FILTERS>(lpMsg->message, (unsigned int*)&lpMsg->lParam, (unsigned int*)&lpMsg->wParam, (intptr_t)lpMsg->hwnd)
...
template<typename X, typename... T>
void SetFilter(SplitPlayMessageFilterIDs filterID, bool enabled)
{
...
void MessageFilterHook::SetFilterEnabled(SplitPlayMessageFilterIDs filterID, bool enabled)
{
	SetFilter<SPLITPLAY_MESSAGE_FILTERS>(filterID, enabled);
}
...
void MessageFilterHook::FilterGui()
{
	...
	auto [enabledPtr, name, desc] = ShowListSelectable<SPLITPLAY_MESSAGE_FILTERS>(selected);

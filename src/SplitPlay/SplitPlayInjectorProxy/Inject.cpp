#include "framework.h"
#include "include/splitplayinejctorproxy.h"
#include <string>
#include <easyhook.h>

extern "C" __declspec(dllexport) int SplitPlayEasyHookRuntimeInjectInternal(unsigned int pid, bool stealthInject)
{
	...
	{
		MessageBoxW(NULL, L"Failed to get folder path", L"SplitPlayInjector Error", MB_OK);
		return -1;
	}

	folderpath = folderpath.substr(0, pos + 1);

	const auto path32 = folderpath + L"SplitPlayHooks32.dll";
	const auto path64 = folderpath + L"SplitPlayHooks64.dll";
